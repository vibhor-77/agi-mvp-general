...
class TestFullPipeline:
    ...
    def test_solves_mirror_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["mirror_h"], "mirror_h")
        assert result["solved"] is True
        assert result["score"] >= 0.99

    def test_solves_rotate_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["rotate_90"], "rotate_90")
        assert result["solved"] is True

    def test_solves_scale_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["scale_2x"], "scale_2x")
        assert result["solved"] is True

    def test_solves_gravity_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["gravity_down"], "gravity_down")
        assert result["solved"] is True

    def test_solves_fill_enclosed_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["fill_enclosed"], "fill_enclosed")
        assert result["solved"] is True

    def test_solves_outline_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["outline_task"], "outline_task")
        assert result["solved"] is True

    def test_solves_color_swap_task(self, solver):
        result = solver.solve_task(SAMPLE_TASKS["color_swap_1_to_2"], "color_swap_1_to_2")
        assert result["solved"] is True

    def test_solves_composition_task(self, solver):
        ...
        assert result["score"] > 0.5  # At least partial solve


class TestKnowledgeCompounding:
    ...
    def test_toolkit_grows_after_solving(self):
        ...
        assert solver.toolkit.size >= initial_size
    ...
    def test_archive_records_solutions(self):
        ...
        assert len(solver.archive.history) >= 2
...
class TestAblationStudies:
    ...
    def test_approximability_matters(self):
        ...
        assert score == 0.75  # 3 out of 4 pixels correct
    ...
    def test_composability_needed_for_multi_step(self):
        ...
        assert result["score"] < 0.99 or result["program_length"] <= 1
